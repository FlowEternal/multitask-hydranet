...
from net.common import SeparableConvBlock, MaxPool2dStaticSamePadding,MemoryEfficientSwish,Swish,Conv2dStaticSamePadding
    ...
    def _forward_fast_attention(self, inputs):
        if self.first_time:
            if len(inputs) ==4:
                ...
                # len = 4
                p3, p4, p5, p6_in_ = inputs[-4:]
                p6_in = self.p6_down_channel(p6_in_)
...
class stackBiFPN(nn.Module):
    def __init__(self, fpn_num_filters, fpn_cell_repeats, conv_channel_coef, onnx_export = False):
        super(stackBiFPN, self).__init__()

        self.fpn_num_filters = fpn_num_filters
        self.fpn_cell_repeats = fpn_cell_repeats
        self.conv_channel_coef = conv_channel_coef # the channels of P3/P4/P5

        self.bifpn = nn.Sequential(
            *[BiFPN(self.fpn_num_filters,self.conv_channel_coef,True if i == 0 else False,attention=True,use_p8=False,onnx_export=onnx_export)
              for i in range(self.fpn_cell_repeats)])
...
if __name__ == '__main__':
    # input features
    # feat1 = torch.randn([1, 24, 160, 160]).to("cuda:0")
    # feat2 = torch.randn([1, 64, 80, 80]).to("cuda:0")
    # feat3 = torch.randn([1, 152, 40, 40]).to("cuda:0")
    # feat4 = torch.randn([1, 376, 20, 20]).to("cuda:0")
    # features = [feat1, feat2, feat3, feat4]
    # conv_channel_coef = [64, 152, 376]  # the channels of P3/P4/P5

    feat1 = torch.randn([1, 24, 160, 160]).to("cuda:0")
    feat2 = torch.randn([1, 64, 80, 80]).to("cuda:0")
    feat3 = torch.randn([1, 152, 40, 40]).to("cuda:0")
    feat4 = torch.randn([1, 376, 20, 20]).to("cuda:0")
    feat5 = torch.randn([1, 936, 10, 10]).to("cuda:0")
    features = [feat1, feat2, feat3, feat4, feat5]
    conv_channel_coef = [64, 152, 376, 936]  # the channels of P3/P4/P5
    ...
    neck = stackBiFPN(fpn_num_filters=fpn_num_filters,
                      ...
                      conv_channel_coef=conv_channel_coef,
                      onnx_export=True).to("cuda:0")
    ...
    for fused_feat in fused_features:
