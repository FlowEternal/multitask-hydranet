...
def calc_params(lane):
    ...
    if n_pt == 2:
        ...
        a_x = lane[0]['x']
        a_y = lane[0]['y']
        b_x = (lane[1]['x'] - a_x) / (h0 + 1e-8)
        b_y = (lane[1]['y'] - a_y) / (h0 + 1e-8)
    ...
    for i in range(n_pt - 2):
        ...
        C.append(h[i + 1])
        dx1 = (lane[i + 1]['x'] - lane[i]['x']) / (h[i] + 1e-8)
        dx2 = (lane[i + 2]['x'] - lane[i + 1]['x']) / (h[i + 1] + 1e-8)
        tmpx = 6 * (dx2 - dx1)
        dy1 = (lane[i + 1]['y'] - lane[i]['y']) / (h[i] + 1e-8)
        dy2 = (lane[i + 2]['y'] - lane[i + 1]['y']) / (h[i + 1] + 1e-8)
        tmpy = 6 * (dy2 - dy1)
        if i == 0:
            C[i] /= (B[i] + 1e-8)
            D_x.append(tmpx / (B[i] + 1e-8) )
            D_y.append(tmpy / (B[i] + 1e-8))
        else:
            base_v = B[i] - A[i] * C[i - 1]
            C[i] /= (base_v + 1e-8)
            D_x.append((tmpx - A[i] * D_x[i - 1]) / (base_v + 1e-8 ) )
            D_y.append((tmpy - A[i] * D_y[i - 1]) / (base_v + 1e-8 ))
    ...
    for i in range(n_pt - 1):
        a_x = lane[i]['x']
        b_x = (lane[i + 1]['x'] - lane[i]['x']) / (h[i]+1e-8) - (2 * h[i] * Mx[i] + h[i] * Mx[i + 1]) / 6
        c_x = Mx[i] / 2
        d_x = (Mx[i + 1] - Mx[i]) / (6 * (h[i]+1e-8))

        a_y = lane[i]['y']
        b_y = (lane[i + 1]['y'] - lane[i]['y']) / (h[i]+1e-8) - (2 * h[i] * My[i] + h[i] * My[i + 1]) / 6
        c_y = My[i] / 2
        d_y = (My[i + 1] - My[i]) / (6 * (h[i]+1e-8))
...
class LaneMetricCore(MetricBase):
    ...
    def __init__(self, *, iou_thresh, lane_width, prob_thresh=None):
    ...
    def __call__(self, gt_result, pr_result, *args, **kwargs):
        ...
        gt_lanes = []
        for line_spec,label in zip(target_spec['Lines'],target_spec['Labels']):
            if len(line_spec) > 0:
        ...
        for line_spec in predict_spec['Lines']:
            if 'score' in line_spec:
                if line_spec['score'] > prob_thresh:
        ...
        # eval params增加尺寸参数
        self.eval_params["eval_width"] = gt_wh["width"]
        self.eval_params["eval_height"] = gt_wh["height"]
        # self.eval_params["eval_width"] = 2560
        # self.eval_params["eval_height"] = 1440

        result = evaluate_core(gt_lanes=gt_lanes,
                               pr_lanes=pr_lanes,
                               gt_wh=gt_wh,
                               pr_wh=pr_wh,
                               hyperp=self.eval_params)
...
class LaneMetric(MetricBase):
    ...
    def __init__(self, *, method, iou_thresh, lane_width, thresh_list=None):
        """Initialize metric params, we use bitwise IOU as the judgment condition.
        ...
        if thresh_list is not None:
            for prob_thresh_spec in thresh_list:
                lane_metric_core = LaneMetricCore(**self.eval_params, prob_thresh=prob_thresh_spec)
                self.metric_handlers.append(lane_metric_core)
        else:
    ...
    def summary(self):
        """Summary all record from result cache, and get performance."""
        # print all metric
        metric_list = [handlers.summary()[self.method] for handlers in self.metric_handlers]
