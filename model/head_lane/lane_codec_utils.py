...
def convert_lane_to_dict(lane_set, sx, sy):
    ...
    for i in range(len(lane_set)):
        ...
        single_line['score'] = lane_set[i].prob
...
def delete_nearby_point(interp_lane, input_width, input_height):
    ...
    for pt in interp_lane:
        if cnt == 0:
            ...
            # 这里把数据增强后超出边界的直接crop掉 TODO
            if  not ( (0 < cur_x < input_width) and (0 < cur_y < input_height) ):
                continue
...
def calc_err_dis_with_pos(l1, l2,use_mean_dist=False):
...
def nms_with_pos(lane_set, thresh, use_mean_dist = False):
    ...
    for n in range(len(lane_sorted)):
        ...
        for t in range(n + 1, len(lane_sorted)):
            dis = calc_err_dis_with_pos(lane_sorted[n], lane_sorted[t],use_mean_dist)
            if dis <= thresh:
                selected[t] = True
    return lanes_result
